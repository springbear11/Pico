...
Core 已通过只读 `RuntimeEvent`/`IRuntimeEventSink` 向 UI 发布 Session、UUT、Node、Attempt、Loop、Barrier、Device 和 ModuleLog 事件；最终状态仍以 `ExecutionReport` 对账。

**问题 3：Pause/Resume 已实现，调试控制仍不完整**

节点边界 Pause/Resume 已从 Core 接通到 Qt UI；Breakpoint、Step Into/Over 和只读 Debug Snapshot 仍需实现。
