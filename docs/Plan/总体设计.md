...
| 问题域 | 引擎承担的职责 | 设计说明 |
|--------|---------------|----------|
| 流程编排 | JSON 描述 Setup → Main → Cleanup 序列 | |
| 资源仲裁 | 多 UUT 对 DMM1/CAN1 的互斥申请与排队 | |
| 批次同步 | 多 UUT 到 Barrier 点后等待、到齐释放 | |
| 错误恢复 | 失败→重试、失败→清理、失败→跳过 | |
| 循环控制 | For 循环由调度层控制，Plan 保持不可变 | |
| 设备连接 | 逻辑设备 ID ↔ 真实地址映射、连接复用 | |
| 模块隔离 | DLL/Python 崩溃不影响引擎进程 | - |
...
| 外部系统 | 当前状态 | 交互方式 |
|----------|----------|----------|
| UI 层 | 规划中 | 通过 ExecutionReport DTO（只读）消费 |
| 设备层 (DMM/CAN/PSU) | Fake 设计已完成，真实硬件待接入 | 通过 IModuleTransport → PersistentQProcessTransport |
| MES | 无 | 当前设计未涉及 |
| 数据库 | 无 | ISessionPersistence 接口占位 |
| Python 脚本 | 设计支持 | QProcessTransport + moduleBindings |
| C/C++ DLL | 设计支持 | PicoATE.NativeHost.exe + PicoATE_Execute C ABI |
...
| 头文件 | 核心类型 | 职责 |
|--------|----------|------|
| | `ExecNode`, `ExecEdge`, `ExecutionPlan`, `CleanupRegion`, `LoopRegion`, `ForLoopSpec` | 不可变执行计划，含节点/边/清理区/循环区 |
| | `UutExecution`, `NodeActivation`, `NodeAttempt`, `ExecutionFrame`, `ExecutionState` | 运行时可变状态，含多 UUT 视图 |
| | `SequenceDef`, `StepDef`, `StepGroupDef`, `ModuleBindingDef` | 编辑期模型，JSON 反序列化产物 |
| | `MeasurementResult`, `MeasurementStatus` | 结构化测量值 DTO |
| | `ExecutionReport`, `UutReport`, `StepReport`, `AttemptReport`, `StepLoopReport` | 只读运行报告 DTO |
| | `ExecutionSessionSnapshot`, `ISessionPersistence` | 会话快照与持久化接口占位 |

**B. 编译与构建层**

| 头文件 | 核心类型 | 职责 |
|--------|----------|------|
| | `SequenceCompiler`, `CompileResult`, `CompileError`, `CompileWarning` | JSON → SequenceDef → ExecutionPlan，含类型校验和未知字段警告 |
| | `PlanBuilder`, `PlanBuildResult`, `PlanBuildError` | SequenceDef → ExecutionPlan 图构建 |
| | `PlanCache` | 基于 shared_ptr 的编译缓存 |

**C. 调度引擎层**

| 头文件 | 核心类型 | 职责 |
|--------|----------|------|
| | `ExecutionSession`, `ExecutionSessionResult` | 多 UUT 执行会话主入口 |
| | `ExecutionGraphScheduler`, `SchedulerResult`, `SchedulerStepResult` | 节点调度：依赖检查→资源获取→执行→错误处理 |
| | `NodeRunner`, `INodeHandler`, `NoopNodeHandler`, `WaitNodeHandler`, `ActionNodeHandler` | 节点分发执行器，Handler 模式 |
| | `ResourceManager`, `ResourceLease`, `ResourceRequest` | 资源互斥申请、排队、释放 |
| | `BarrierController`, `BarrierRuntimeState`, `BarrierReleaseDecision` | 多 UUT 批次同步（6 种到达策略 × 4 种释放策略 × 5 种失败策略） |
| | `LoopController`, `LoopControllerResult` | For 循环游标控制 |
| | `ErrorPolicyEngine`, `ErrorDecision` | 错误策略决策（Continue/StopUut/Retry/RunCleanup/Abort） |

**D. 模块运行时层**

| 头文件 | 核心类型 | 职责 |
|--------|----------|------|
| | `IModule`, `IModuleTransport`, `ModuleRegistry`, `TransportModuleAdapter`, `ModuleExecutionContext`, `ModuleResult`, `IModuleRuntimeServices` | 模块执行、传输抽象、设备服务注入 |
| | 同上含 `ActionNodeHandler` | 将模块调用嵌入节点执行 |
| | JSON 序列化/反序列化函数 | Transport 请求/响应 ↔ JSON |
| | `registerConfiguredModules()` | JSON moduleBindings → 运行时模块注册 |

**E. Transport 实现层**

| 头文件 | 核心类型 | 职责 |
|--------|----------|------|
| | `QProcessTransport` | 短生命周期子进程 transport |
| | `PersistentQProcessTransport` | 长驻子进程 transport，跨调用保持状态 |
| | `DllBridgeInvoker` | 进程内 QLibrary DLL 调用（原型验证用） |
| | `NativeHostManifest`, `loadNativeHostManifest()` | DLL 加载清单 JSON 解析 |

**F. 设备管理层**

| 头文件 | 核心类型 | 职责 |
|--------|----------|------|
| | `DeviceSessionManager`, `IDeviceSession`, `IDeviceSessionFactory`, `DeviceSessionConfig`, `DeviceSessionLifetime` | 逻辑设备注册、连接复用、状态管理 |
| | `TransportDeviceSession`, `TransportDeviceSessionFactory` | 将 Transport 映射为设备 session |
| | `StationConfig`, `parseStationConfigJson()`, `configureDeviceSessions()` | 工站配置 JSON 解析 |
| | `StationRuntime` | 工站运行上下文，持有 StationConfig + DeviceSessionManager |
| | `ExampleDmmAdapterModule`, `ExampleCanAdapterModule` | DMM/CAN 适配器 Spike 实现 |

**G. 变量替换层**

| 头文件 | 核心类型 | 职责 |
|--------|----------|------|
| | `VariableResolver`, `VariableResolverOptions` | 配置期 ${PROJECT_DIR} 等替换，支持递归和容器 |
| | `RuntimeVariableResolver`, `RuntimeVariableContext` | 运行期 ${var.NAME}/${loop.index}/${uut.id} 替换 |

#### src/cli/ — 命令行入口

：唯一入口文件（517 行）。组装整个 pipeline：编译 JSON → 加载 Station Config → 创建 ExecutionSession → 注册模块 → 运行 → 打印报告。

#### src/mockhost/ — 测试用 Echo 进程

（72 行）：stdin 读一行 JSON → echo 回去。验证 QProcessTransport 协议。

#### src/nativehost/ — DLL 隔离加载器

（207 行）：stdin 读请求 → QLibrary 加载 DLL → 调用 PicoATE_Execute → stdout 写响应。支持 --manifest 和 --dll 两种模式。

#### src/fakeinstrumenthost/ — 假仪器长驻进程

（320 行）：保持进程运行，累积设备状态（openCount/readCount），支持 open/read/configureDcv/close/health/reconnect/shutdown 命令。验证 PersistentQProcessTransport 跨 step 状态保持。

#### src/testdllmodule/ — C ABI 测试 DLL

（80 行）：实现 `PicoATE_Execute` C ABI。支持 dllSleepMs（超时测试）、dllReturnCode（错误测试）、outputs/measurements 回传。

#### src/canexamplemodule/ — 模拟 CAN 解码 DLL

（247 行）：解析 rawBytes（hex string/array）→ 按 signal 定义解码（大小端/有无符号/scale/offset）→ 限值判定 → 返回 MeasurementResult。纯软件验证，不需要 CAN 分析仪。
...
| **管道（Pipeline）** | JSON → SequenceDef → ExecutionPlan → ExecutionSession → ExecutionReport | → → |
| **策略模式** | Barrier 的 6 种到达策略、4 种释放策略、错误恢复的 5 种动作 | 、|
| **注册/插件** | IModule 通过 ModuleRegistry 注册，IDeviceSessionFactory 通过 DeviceSessionManager 注册 | 、|
| **状态机** | ExecutionState（9 个状态）、ActivationState（11 个状态）、FrameState、AttemptState | |
...
| 机制 | 实现 | 代码位置 |
|------|------|----------|
| **资源互斥** | ResourceManager 的 tryAcquire/release，支持 Exclusive/SharedRead/SharedWrite/Counted 四种模式 | |
| **等待队列** | 资源申请失败时加入 waiter 队列，资源释放时不自动唤醒（需下一轮重试） | |
| **Barrier 联合** | 多 UUT 到 Barrier 点后等待，满足条件后统一释放 | |
| **停止保护** | ExecutionSession 的 stopRequested 标志 + skipPendingNonAlwaysRun | |
...
ExecutionReport 作为 UI 消费的 DTO 已就绪。CLI 是当前唯一的"用户界面"。
...
| DllBridgeInvoker（直接 QLibrary） | 进程内（原型验证） | TestDllModule |
...
关键证据：
- ：IModule 接口只接受 `ModuleExecutionContext` 和 `ModuleFunction`，返回 `ModuleResult`
- ：`NodeOutcome toNodeOutcome(ModuleOutcome)` — 引擎层负责转换，业务层不知道 `NodeOutcome`
- ：UI 消费的结构是纯数据 DTO
...
|----------|------|------|------|------|------|----------|
| 编译 | `SequenceCompiler::compileJson()` | QJsonObject | CompileResult | 否 | JSON 编译为 SequenceDef + ExecutionPlan | |
| 构建 | `PlanBuilder::build()` | SequenceDef | PlanBuildResult | 否 | SequenceDef → ExecutionPlan | |
| 会话创建 | `ExecutionSession::ExecutionSession()` | ExecutionPlan | - | 否 | 创建执行会话 | |
| UUT 注册 | `ExecutionSession::addUut()` | UutId | UutExecution& | 否 | 添加被测试单元 | |
| 模块注册 | `ExecutionSession::registerModule()` | shared_ptr\<IModule\> | bool | 否 | 注册业务模块 | |
| 运行 | `ExecutionSession::run()` | - | ExecutionSessionResult | 否 | 启动执行 | |
| 停止 | `ExecutionSession::requestStop()` | StopMode | void | 否 | 请求停止（Graceful/Abort） | |
| 状态查询 | `ExecutionSession::state()` | - | ExecutionState | 否 | 查询会话状态 | |
| 报告 | `ExecutionSession::report()` | - | ExecutionReport | 否 | 生成只读运行报告 | |
| 快照 | `ExecutionSession::snapshot()` | - | ExecutionSessionSnapshot | 否 | 生成可序列化快照 | |
| 设备管理 | `ExecutionSession::devices()` | - | DeviceSessionManager& | 否 | 获取设备管理器引用 | |
| UUT 查询 | `ExecutionSession::uuts()` | - | QVector\<UutExecution\>& | 否 | 获取 UUT 执行数据 | |
| 工站加载 | `StationRuntime::loadStationConfigFile()` | filePath, options | StationRuntimeResult | 否 | 加载工站配置 | |
| 模块注册 | `registerConfiguredModules()` | session, sequence, options | ModuleBindingRegistrationResult | 否 | 从序列定义注册模块 | |
...
| 设计模式 | 代码位置 | 解决什么问题 | 评价 |
|----------|----------|-------------|------|
| **策略模式** | BarrierController 的 6×4×5 策略组合 、ErrorPolicyEngine 的 5 种错误动作  | Barrier 的到达/释放/失败/超时行为可从 JSON 配置切换，不需要改调度器 | 很典型的策略应用，通过枚举+switch 而非虚函数，性能好但增加新策略需改枚举 |
| **工厂方法** | IDeviceSessionFactory 、TransportDeviceSessionFactory  | 设备 session 的创建延迟到运行时，由 driver 注册的工厂负责 | 标准抽象工厂用法 |
| **适配器模式** | TransportModuleAdapter  把 IModuleTransport 适配为 IModule | 外部进程/DLL 对调度器来说看起来和内置模块一样 | 这是整个模块隔离的核心 |
| **注册表模式** | ModuleRegistry 、DeviceSessionManager 中的 factory 注册  | 支持运行时动态注册模块/设备，编译期不需要知道所有实现 | 经典的插件注册机制 |
| **建造者模式** | PlanBuilder  将 SequenceDef 逐步构建为 ExecutionPlan | 编译阶段的关注点分离，buildGroup/buildStep/buildLoopStep 分层构建 | 构建过程清晰，每个方法职责单一 |
| **处理器链** | NodeRunner 中的 INodeHandler 链 ：NoopHandler → WaitHandler → ActionHandler | 不同节点类型由不同 Handler 处理，可以通过 registerHandler() 扩展 | 轻量级责任链，新增节点类型只需注册新 Handler |
...
| **不可变对象** | ExecutionPlan 构建后只读（const shared_ptr） | 多 UUT 共享同一个 Plan 无需加锁；缓存安全 | 重要的并发安全设计决策 |
| **DTO** | ExecutionReport 及其子结构  | UI 只消费数据，不接触运行时对象 | 三层解耦的关键 |
...
| 异常类型 | 处理机制 | 代码位置 |
|----------|----------|----------|
| **步骤失败** (Failed) | ErrorPolicyEngine 根据 onFail 策略处理 | |
| **模块错误** (Error) | ErrorPolicyEngine 根据 onError 策略处理 | 同上 |
| **执行超时** (Timeout) | ErrorPolicyEngine 根据 onTimeout 策略处理；Transport 层有独立 timeout | + 各 Transport |
| **进程崩溃** (非零退出) | QProcessTransport 捕获 → ModuleTransportStatus::TransportError | QProcessTransport |
| **DLL 超时** | DllBridgeInvoker 线程超时 → DllExecuteTimeout（不能安全杀线程） | |
| **用户停止** | requestStop() → skipPendingNonAlwaysRun → activateAllCleanup | |
| **资源获取超时** | acquireTimeoutMs 配置（当前代码中定义了但未在 Scheduler 中强制检查） | |
...
- **Step 级资源**（通过 ResourceManager）：在 executeNode() 的 finally 逻辑中保证释放  统一释放
...
| 防护机制 | 实现 |
|----------|------|
| 节点级别超时 | TimeoutPolicy.timeoutMs（在 ExecNode 定义，但当前代码中 NodeRunner 未实际检查——**设计阶段尚未覆盖是否已完整实现**） |
...
- ：`while (progressed)` 直接在调用线程
- ：`QThread::msleep()` 阻塞当前线程
- DllBridgeInvoker：使用 QThread 做 DLL 调用隔离（唯一的子线程使用）
...
| 机制 | 实现 | 评价 |
|------|------|------|
| traceId | TransportModuleAdapter 自动生成 `moduleId:hex` 格式 | 中 traceId 字段已定义 |
...
`uuts()` 返回 `QVector<UutExecution>&`（可变引用），外部可以修改运行时状态，违反了封装原则。应该只暴露 const 版本或通过 ExecutionReport 访问。

**问题 5：DeviceSessionManager 直接暴露可变引用**

`devices()` 返回 `DeviceSessionManager&`，允许外部绕过 Session 直接操作设备。
...
| 类名 | 文件 | 职责 |
|------|------|------|
| ExecutionPlan | | 不可变执行计划 |
| UutExecution | | 单 UUT 运行时状态 |
| NodeActivation | | 节点激活状态 |
| NodeAttempt | | 单次执行尝试 |
| ExecutionSession | | 多 UUT 执行会话 |
| ExecutionGraphScheduler | | 节点调度器 |
| NodeRunner | | 节点分发执行器 |
| ResourceManager | | 资源仲裁 |
| BarrierController | | 批次同步 |
| LoopController | | 循环控制 |
| ErrorPolicyEngine | | 错误策略决策 |
| SequenceCompiler | | JSON 编译器 |
| PlanBuilder | | 执行计划构建器 |
| ModuleRegistry | | 模块注册表 |
| TransportModuleAdapter | | Transport→Module 适配器 |
| DeviceSessionManager | | 设备连接管理 |
| StationRuntime | | 工站运行上下文 |
| VariableResolver | | 配置期变量替换 |
| RuntimeVariableResolver | | 运行期变量替换 |
| ExecutionReport | | 只读运行报告 |
...
|------|------|--------|------|
| IModule | | 2 | 业务模块接入 |
| IModuleTransport | | 1 | Transport 抽象 |
| INodeHandler | | 2 | 节点处理器 |
| IModuleRuntimeServices | | 4 | 设备服务注入 |
| IDeviceSession | | 7 | 设备会话 |
| IDeviceSessionFactory | | 2 | 设备会话工厂 |
| IDeviceCommandSession | | 1 | 设备命令代理 |
| ISessionPersistence | | 2 | 会话持久化（占位） |
...
| CleanupReason | 8 | |
| ModuleOutcome | 4 | |
| ModuleTransportStatus | 3 | |
| MeasurementStatus | 5 | |
| BarrierArrivalPolicy | 6 | |
| BarrierReleasePolicy | 4 | |
| BarrierFailurePolicy | 5 | |
| BarrierTimeoutPolicy | 5 | |
| DeviceSessionLifetime | 3 | |
| DeviceConnectionState | 4 | |
...
**文档结束。** 本架构设计文档基于 PicoATE 设计文档完整分析得出。标注"设计阶段尚未覆盖"的项目代表设计中尚未明确的部分。
