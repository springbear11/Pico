...
add_bullet('已通过只读 RuntimeEvent/IRuntimeEventSink 向 UI 发布运行进度和业务日志')
add_bullet('Pause/Resume 已使用线程安全 ExecutionControl 和节点边界安全点实现')
add_bullet('尚未实现 Breakpoint、Step Into/Over 和只读 Debug Snapshot')
...
add_note('Pause/Resume 已实现为 Session 级协作式控制。ExecutionControl 可跨线程请求暂停，ExecutionSession 在 pumpOnce 前确认节点边界安全点；当前 Step 不被强制中断。暂停保留 UUT、Barrier 和等待状态，Stop/Abort 可唤醒暂停并继续 Cleanup。')
...
add_para('问题 2：事件通知机制已完成基础接入', bold=True)
add_para('Core 已通过只读 RuntimeEvent/IRuntimeEventSink 向 UI 发布运行事件；最终状态仍以 ExecutionReport 对账。')

add_para('问题 3：Pause/Resume 已实现，调试控制仍不完整', bold=True)
add_para('节点边界 Pause/Resume 已从 Core 接通到 Qt UI；Breakpoint、Step Into/Over 和只读 Debug Snapshot 仍需实现。')
...
add_table(
    ...
    [
        ...
        ['Breakpoint 与单步调试', '中', 'Pause/Resume 已完成，继续复用节点边界安全点'],
...
# ── Save ────────────────────────────────────────────────────────
output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'PicoATE任务引擎架构设计.docx')
