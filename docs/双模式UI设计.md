...
Admin 登录同时支持固定工程密码和当天动态口令，任意一个正确即可进入：
...
动态口令在每次校验时根据本机当天日期计算，不写入设置文件；固定密码也不会保存到用户配置。两种密码都用于防止产线人员误操作，不作为高安全等级身份认证。
...
## 12. 2026-07-22 当前交互规则

- Admin 登录完成后自动扫描并加载根目录 `plugins` 注册表，Station 的 Driver 和 Flow 的功能目录不再依赖手动先点一次扫描。
- 未选择目标设备时，Plugin Functions 展示全部插件，方便查看能力；属于 Station 设备类别的功能在拖入前必须先选择设备，算法、文件处理等非设备插件可以直接拖入。
- 选择目标设备后，插件目录只展示相同设备类别的功能；Basic Functions 始终展示。
- 选择物理设备后默认选中第一个启用通道，再次点击同一设备会取消目标选择。
- 插件 Step 的 Function 由 `PicoATE_Describe` 生成下拉框，不再要求手写函数名。
- 点击 Run 后自动切换到 Run Test 页；编译诊断定位、运行定位等程序内部跳转不会弹保存提示。
- Flow 的普通编辑先保存在内存草稿中，离开 Flow 页时统一询问；`Ctrl+S` 或保存按钮明确写入文件。
- Undo/Redo 后通过稳定节点身份重新定位编辑对象。找不到旧节点时清除选择并允许继续保存，避免出现必须退出后才能恢复的死锁状态。
- 文档内容没有实际变化时，保存视为成功，不再因“无可提交修改”阻止关闭或切页。

## 13. SN 扫码规则（2026-07-22）

- Station 可组合配置固定长度、通配模式和允许字符正则；留空字段不启用对应限制。
- 通配模式支持 `BTSN*`、`*BTSN*`、`*BTSN`，用于前缀、中间字段和后缀产品系列。
- 常规大写字母和数字可配置为 `^[A-Z0-9]+$`，特殊项目可以改用自己的完整正则。
- TEST 自动扫码与 Admin 手动扫码执行相同规则；错误时扫码框保持打开并聚焦原输入。
- SN 不做历史去重。通过后保存到当前 UUT 运行变量，测试结束后仍作为报告文件名和界面产品标识。
