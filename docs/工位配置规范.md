...
| `name` | string | 否 | 工站显示名。 |
| `stopOnFailure` | bool | 否 | 默认 `true`。开启时测试失败后进入既有停止/清理流程；关闭时允许继续下一个最外层测试项。TestItem 内部仍保持首个子步骤失败后跳过剩余子步骤。 |
...
- 更换 GCAN/ZLG 等型号时，优先只修改 Station 绑定；Sequence 中的 `CAN1` 和业务流程保持不变。
- 同一插件型号可以绑定多个逻辑设备。Flow Editor 会显示 `Send [CAN1]`、`Send [CAN2]` 等明确目标；拖入后分别写入对应的 `deviceId`。只有一个设备时不显示后缀。
