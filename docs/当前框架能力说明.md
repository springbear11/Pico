...
- GCAN 与创芯 CAN 插件、真实双盒交叉连接脚本和硬件日志链路已经落地；Modbus TCP
  插件已纳入统一注册表，覆盖 FC01/02/03/04/05/06/0F/10；
- 通用数据解析支持 CAN/二进制字段、数值寄存器、ASCII/UTF-8 寄存器文本、起止符、
  分隔符和正则捕获；FC10 写扫码 SN 后可用 FC03 回读并做字符串一致性判断；
- TXT、CSV、XLSX、PDF 报告及 PASS/FAIL 归档已经实现；多 UUT 运行可按 UUT 物理拆分
  文件并独立判定归档，共享 Setup/Cleanup 会进入每份产品报告；
- Debug 配置最近一次完整 CTest 基线为 `23/23` 通过。

当前仍不能称为完整量产版本，主要缺口是：发布安装与干净机器验收、各类真实设备的
长时间稳定性与断线恢复验收、Checkpoint 恢复、用户权限和限值数据库。
...
| 动作 | 意思 |
|------|------|
| `Inherit` | 继承父 TestItem 或 Station 的默认策略。 |
| `Continue` | 继续跑后面的。 |
| `StopUut` | 停掉当前 UUT。 |
| `Retry` | 当前节点再跑一次。 |
| `JumpTo` | Retry 用尽后跳到同级的某个后续步骤，中间步骤记为 Skipped。 |
...
实际项目默认使用 Station 的 `stopOnFailure` 作为继承策略：开启时，未单独配置的
Fail/Error/Timeout 在 Retry 用尽后停止；关闭时继续。工程师也可以在 Flow Editor
给某个 Step 或 TestItem 单独选择策略。显式策略优先于 Station，`JumpTo` 还会提供
后续同级步骤下拉框，避免手写目标 ID。

例如 TestItem 001 失败时选择 `JumpTo -> 003`，调度器会保留 001 的失败结果，把
002 及其子步骤标记为 Skipped，然后执行 003。当前只允许同一父级、同一执行阶段的
向后跳转，不允许跨层级、跨 Cleanup 或进入/离开 Loop body。
...
- 失败跳转会显式记录被跨过的 Skipped 步骤，不会悄悄改写原失败结果。

### 9.4 还缺什么

- 没有 step -> group -> sequence -> station 多级 fallback。
- 没有复杂错误分类。
- JumpTo 暂不支持跨层级、跨执行阶段或 Loop body 内跳转。
- 没有错误码体系。
...
现在日常配置不再要求填写 UUT 数量、Quorum、超时策略等一整套参数。从 Flow 的 Basic Functions 拖入 Barrier 即可，最多只填一个可选名称。参与数量直接使用本次 Session 的实际 UUT 集合；单 UUT 会立即通过。
...
- Failed、Error、Timeout 或 Cancelled 的成员在到达前退出时，会自动从等待集合中移除。
- snapshot 结构。
...
- 先到者帮助后到者这种资源协作。
- barrier 时间线诊断。
- 独立的 Barrier 等待成员和释放状态面板。

当前稳定模式是 `DropFailed + RemoveFailedMember + Lockstep`。旧 JSON 中的高级策略字段仍兼容保留，但 Flow 不再默认展示；Quorum、RollingWindow、人工裁决等高级策略在运行语义完全落地前，不作为正式配置能力宣传。
...
- 每个 UUT 独立的 `serialNumber/completed/outcome/startedAt/finishedAt/durationMs/steps`
...
- wasError 能帮助判断是否错误。
- 任意数量 UUT 的 Main 结果保持独立数据，公共 Setup/Cleanup 在内存模型中不重复拷贝。
- RuntimeEvent 日志保留 `uutId`，可按 UUT 独立分流；公共阶段日志单独标识。
- TEST/Admin 落盘器按实际 UUT 数量生成独立 TXT/CSV/XLSX/PDF；单个 UUT 失败只影响
  自己的文件归档，共享阶段失败或 Stop/Abort 会让全部 UUT 文件进入 FAIL。
- 每份物理报告包含本 UUT 的 Main 和共享 Setup/Cleanup，既避免跨 UUT 串数据，也能
  单独拿一份文件完成诊断。
...
- 当前 TEST/Admin 视觉界面仍以单 UUT 为主，尚未提供四块独立实时结果树和日志区；
  这属于 UI 展示缺口，不影响底层多 UUT 报告数据和物理文件拆分。
...
- 已有 TXT、CSV、XLSX、PDF 生产报告，但没有中心数据库、MES 上传和限值数据库。
- 已完成 GCAN/CX CAN 真实硬件脚本与插件接入；DMM、PSU 和 Modbus TCP 插件已有硬件调试基础，
  仍需按交付工站做长时间稳定性、掉线和恢复验收；SCOPE 仍待真实设备接入。
...
## 33.9 2026-08-24 多 UUT 共享测试项

- Flow 的普通 Step 或顶层 TestItem 可以选择 `Per UUT` 或 `Once per batch`，默认仍是每个 UUT 分别执行。
- `Once per batch` 会等待本批次仍有效的 UUT 全部到达，只调用一次插件，然后把最终结果复制到每个 UUT 的运行状态和结果仓库。
- 共享 TestItem 的全部子步骤与整体 Retry 只执行一套，不会因为 4 个 UUT 把公共上电、复位或人工确认重复做 4 次。
- 共享步骤仍出现在每个 UUT 的独立报告中；attempt、measurement、outputs、错误码、PASS/FAIL 都保留，报告不会因为物理上只执行一次而缺项。
- 共享失败会按配置的失败策略作用于每个参与 UUT；提前失败并已退出正常流程的 UUT 不再阻塞共享会合点。
- 编译器会拒绝容易造成歧义或死锁的组合，包括 Per-UUT 输入、普通 Per-UUT 前序结果、Loop/Barrier/Break、LOCK/UNLOCK 区间和嵌套共享配置。
- 这项能力不是 Barrier 或互斥锁的别名：Barrier 等齐后仍各跑一次，事务锁让 UUT 依次运行，批次共享才是等齐后只运行一次并广播结果。
