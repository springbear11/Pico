...
- Flow 编辑路径改用稳定节点身份；模型重建、Undo/Redo 后重新定位有效 `QModelIndex`，旧节点失效时主动清除选择。
- 无实际修改时的保存视为成功，避免非法值撤销后无法保存或关闭。
- 新增保存/不保存窗口生命周期及编辑恢复测试；当前全套自动测试 17/17 通过。
...
## UI-TECH-003：Flow 属性编辑期间的概率性控件生命周期崩溃

状态：观察中，等待保留同版本现场再次复现（2026-08-12 记录）。

本次证据：

- Dump：`PicoATE.UI_20260812_162854_627_820.dmp`；
- SHA-256：`60833922A49500BF4B1ADE57E459A6AF0E569741A1E01F2B911BC0F691021103`；
- 异常：主 UI 线程发生 `0xC0000005` 读取访问冲突，读取地址为
  `0xFFFFFFFFFFFFFFFF`；
- 故障点位于 Qt 共享指针保护逻辑
  `QtSharedPointer::ExternalRefCountData::getAndRef(QObject const*)`，上层正在分发
  `QMouseEvent`；
- Dump 中事件接收对象已不是有效的 `QObject`，符合“鼠标事件仍在分发时，目标控件被同步删除”
  的悬空对象特征。

当前最高概率触发链：

1. 工程师在 Flow 属性区点击或切换插件输入控件；
2. `moduleId` 编辑框失去焦点并同步发出 `editingFinished`；
3. `StepPropertyEditor::rebuildCallEditors()` 进入
   `rebuildPluginInputEditors()`；
4. 动态表单旧行使用立即 `delete` 删除标签和输入控件；
5. Qt 返回原鼠标事件分发流程时，再次保护已经删除的目标控件并崩溃。

相关高风险代码位于 `ui/src/StepPropertyEditor.cpp`：`editingFinished` 与
`rebuildCallEditors` 的同步连接，以及 `rebuildPluginInputEditors()` 中对动态控件的立即删除。
当前 Dump 没有配套 Release PDB 和同进程 action log，因此可以高置信确认“UI 控件生命周期”
这一类原因，但不能仅凭本次材料百分之百确认具体点击了哪个字段。现阶段按用户决定只记录、继续观察，
暂不为了一个尚未稳定复现的路径修改正式逻辑。

再次复现时必须整套保留：

- 当时使用的 `PicoATE.UI.exe`、`PicoATECore.dll`、Qt DLL 和对应 PDB；
- 新生成的 Dump，以及该进程启动时创建的完整 `.actions.log`；
- 当时的 Sequence、Station、ProductRouting、PluginRegistry 和相关插件版本；
- 崩溃前最后几步操作，最好附屏幕录像；
- 对应 Git commit，确保分析代码与现场二进制完全一致。

后续若稳定复现，优先验证将动态控件销毁延后到当前事件循环结束、避免在
`editingFinished` 中同步重建被点击控件，并补充属性编辑器焦点切换压力测试。

## UI-TECH-002：插件功能与目标设备类别的静态校验

状态：待实现（2026-07-22 记录）。

当前 Flow Editor 已通过目标设备筛选和拖放门禁，阻止工程师在正常新建路径中把 CAN 功能绑定到
PSU 等不兼容设备。但旧 JSON、手工编辑文件或后续修改 Station Type，仍可能留下类别不匹配的绑定。

后续需要在编辑器和编译器增加同一套诊断：

- 根据插件 Describe 的类别和 Station 目标设备 Type 判断兼容性；
- 在 Flow 当前页直接显示诊断，并高亮目标设备字段和对应 Step；
- 编译阶段继续阻断不兼容绑定，避免问题拖到 Run 时才暴露；
- 对没有设备类别的算法、文件处理等插件保持直接调用能力。
...
## MULTI-UUT-001：共享硬件故障的整批停止策略

当前已经实现默认策略：普通 Step/TestItem 失败只影响当前 UUT，其他 UUT 继续，最后
统一执行一次 Session Cleanup。后续需要区分“产品自身失败”和“共享工装/总线/仪器失效”：

- 产品失败继续隔离在当前 UUT；
- 共享硬件致命故障可配置为停止整批；
- 整批停止前必须定义正在执行、等待资源和未开始 UUT 的确定性终态；
- 不能只靠插件返回 `Error` 猜测故障范围，需要结构化错误分类或资源故障标识。

资源事务区间已完成；在故障来源分类完成前，仍不提供表面可选但语义不完整的
`StopBatch` 开关。

## MULTI-UUT-003：资源区间恢复与诊断

资源区间已经支持正常退出、失败跳过、Stop 和 Main 收尾释放。后续 Checkpoint 恢复
需要持久化“哪个 UUT 正持有哪些区间租约”，恢复时不能直接复用失效的系统句柄，而应
按稳定资源 ID 重新仲裁。运行报告还需要补充等待、取得和释放资源的专用事件，便于诊断
共享硬件排队耗时。

## MULTI-UUT-002：四路独立 UI 与文件落盘

Core 已提供四份独立 `UutReport`，RuntimeEvent/RuntimeLogLine 已保留 `uutId`，公共
Setup/Cleanup 使用 Session 分区。尚未完成：

- TEST/Admin 四块独立结果树和日志区；
- 每个 UUT 独立 TXT、CSV、XLSX、PDF 文件及命名规则；
- 公共 Setup/Cleanup 日志和报告是单独文件还是附加到每个 UUT 文件的产品规则；
- 四路独立良率、平均时间和历史查询。
