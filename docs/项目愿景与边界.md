...
- 一个插件 DLL 对应一个具体设备实现，例如 GCAN、ZLG 分别生成 DLL。
- 同类插件使用统一功能 ID，例如 CAN 的 open/write/read/close；厂商特有能力可以额外增加。
- DLL 通过 `PicoATE_Describe` 只描述自身功能、必填/选填参数和输出，不连接硬件。
- UI 通过 NativeHost 隔离扫描 DLL，并把结果缓存到统一 `PluginRegistry.json`。
- `StationSystem.json` 使用 CAN1、DMM1 等逻辑设备绑定具体型号，Sequence 优先引用逻辑设备，
  避免在每个 Step 中写死 GCAN/ZLG。
- 更换同类设备时优先只修改 Station 绑定和底层插件；只有厂商特有必填参数无法兼容时才调整流程参数。
