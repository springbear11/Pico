...
class Hdm3000Adapter::Impl
{
public:
    Detail::VisaScpiSession session;
};

Hdm3000Adapter::Hdm3000Adapter()
    : m_impl(std::make_unique<Impl>())
{
}

Hdm3000Adapter::~Hdm3000Adapter() = default;

Result Hdm3000Adapter::connect(const std::string& visaAddress, const Plugin::Json& options)
{
    return m_impl->session.connect(visaAddress, options);
}

void Hdm3000Adapter::disconnect() noexcept
{
    m_impl->session.disconnect();
}

bool Hdm3000Adapter::isConnected() const noexcept
{
    return m_impl->session.connected();
}

Result Hdm3000Adapter::identity()
{
    return m_impl->session.query("*IDN?");
}

Result Hdm3000Adapter::reset()
{
    return m_impl->session.write("*RST");
}

Result Hdm3000Adapter::clear()
{
    return m_impl->session.clear();
}

Result Hdm3000Adapter::configure(MeasurementMode mode, double range, double integration)
{
    const auto configuration = Detail::commandFor(mode, range);
    if (configuration.empty()) return Result::failed("UnsupportedMeasurementMode", "HDM3000 mode is not supported");
    ...
    PicoATE_Log("HDM3000_CONFIG command={} integration={}", configuration, integration);
    return Result::passed();
}

Result Hdm3000Adapter::read()
{
    return m_impl->session.numericQuery("READ?");
}

Result Hdm3000Adapter::query(const std::string& command)
{
    return m_impl->session.query(command);
}

Result Hdm3000Adapter::write(const std::string& command)
{
...
{
    return std::make_unique<Hdm3000Adapter>();
}
