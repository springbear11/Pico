               ...
               const PicoATE::Core::MeasurementResult* measurement)
{
    ...
    const QStringList cells = {
        ...
        attemptOutcome,
        attempt && attempt->durationMs >= 0 ? QString::number(attempt->durationMs) : QString(),
    ...
    const PicoATE::Core::ExecutionReport& report)
{
    ...
    csv += "\"UUT\",\"Step ID\",\"Step Name\",\"Step Outcome\",\"Step Duration Ms\","
           "\"Attempt\",\"Attempt Outcome\",\"Attempt Duration Ms\","
           "\"Loop Iteration\",\"Loop Value\",\"Measurement\","
           "\"Value\",\"Unit\",\"Lower Limit\",\"Upper Limit\",\"Measurement Status\","
           "\"Error Code\",\"Error Message\"\r\n";
    for (const auto& uut : report.uuts) {
        for (const auto& step : uut.steps) {
            appendStepCsv(csv, uut, step);
        }
    }
